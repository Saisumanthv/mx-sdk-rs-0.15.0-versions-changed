...
pub fn generate_setter_impl(m: &Method, identifier: &str) -> proc_macro2::TokenStream {
	...
	quote! {
		#msig {
			#key_snippet
			dharitri_wasm::storage_set(self.get_storage_raw(), &key[..], & #pat);
		}
	}
}
	...
	match m.return_type.clone() {
		...
		syn::ReturnType::Type(_, ty) => {
			quote! {
				#msig {
					#key_snippet
					<#ty as dharitri_wasm::storage::mappers::StorageMapper<Self::Storage>>::new(
						self.get_storage_raw(),
						dharitri_wasm::types::BoxedBytes::from(key),
					)
				}
			}
		},
...
pub fn generate_is_empty_impl(m: &Method, identifier: &str) -> proc_macro2::TokenStream {
	...
	quote! {
		#msig {
			#key_snippet
			dharitri_wasm::api::StorageReadApi::storage_load_len(&self.get_storage_raw(), &key[..]) == 0
		}
	}
}
...
pub fn generate_clear_impl(m: &Method, identifier: &str) -> proc_macro2::TokenStream {
	...
	quote! {
		#msig {
			#key_snippet
			dharitri_wasm::api::StorageWriteApi::storage_store_slice_u8(&self.get_storage_raw(), &key[..], &[]);
		}
	}
}
