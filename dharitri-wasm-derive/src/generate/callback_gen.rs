...
fn generate_callback_body_regular(methods: &[Method]) -> proc_macro2::TokenStream {
	...
	if match_arms.is_empty() {
		...
		quote! {
			let ___tx_hash___ = dharitri_wasm::api::BlockchainApi::get_tx_hash(&self.api);
			...
			let mut ___cb_data_deserializer___ = dharitri_wasm::hex_call_data::HexCallDataDeserializer::new(___cb_data_raw___.as_slice());
...
pub fn generate_callback_proxies(methods: &[Method]) -> proc_macro2::TokenStream {
		...
		.filter_map(|m| {
			if matches!(m.public_role, PublicRole::Callback) {
				...
				let proxy_decl = quote! {
					pub fn #method_name ( &self , #(#arg_decl),* ) -> dharitri_wasm::types::CallbackCall{
						let mut ___closure_arg_buffer___ = dharitri_wasm::types::ArgBuffer::new();
						#(#arg_push_snippets)*
						dharitri_wasm::types::CallbackCall::from_arg_buffer(#cb_name_literal, &___closure_arg_buffer___)
					}
	...
	quote! {
		...
		where
			...
			A: dharitri_wasm::api::ErrorApi + Clone + 'static,
		{
			pub api: A,
