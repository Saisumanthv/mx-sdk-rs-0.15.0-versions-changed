...
pub fn contract_implementation(
	...
	let function_selector = quote! {
	  impl <T, BigInt, BigUint> dharitri_wasm::api::CallableContract<T> for #contract_impl_ident<T, BigInt, BigUint>
	  #api_where
	  {
		...
		fn clone_contract(&self) -> Box<dyn dharitri_wasm::api::CallableContract<T>> {
	  ...
	  impl <T, BigInt, BigUint> dharitri_wasm::api::ContractWithAbi for #contract_impl_ident<T, BigInt, BigUint>
	  #api_where
	  {
		type Storage = T::Storage;

		fn abi(&self, include_modules: bool) -> dharitri_wasm::abi::ContractAbi{
			#abi_body
		}
	  }
	};
