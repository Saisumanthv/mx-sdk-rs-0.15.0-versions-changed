...
#[dharitri_wasm_derive::contract(PingPongImpl)]
pub trait PingPong {
	/// Necessary configuration when deploying:
	/// `ping_amount` - the exact MOAX amounf that needs to be sent when `ping`-ing.
	...
	/// User sends some MOAX to be locked in the contract for a period of time.
		...
		match user_status {
			...
			UserStatus::Registered => {
				...
				if let Some(user_address) = self.user_mapper().get_user_address(user_id) {
					self.send()
						.direct_moax(&user_address, &self.ping_amount().get(), b"pong");
					Ok(())
				} else {
