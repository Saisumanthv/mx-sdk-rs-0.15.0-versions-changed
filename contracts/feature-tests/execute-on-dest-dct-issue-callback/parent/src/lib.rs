...
#[dharitri_wasm_derive::callable(ChildProxy)]
pub trait Child {
	fn issueWrappedMoax(
		&self,
	...
	) -> ContractCall<BigUint, ()>; // payable MOAX
}

#[dharitri_wasm_derive::contract(ParentImpl)]
pub trait Parent {
	#[init]
	fn init(&self) {}
	...
	fn execute_on_dest_issue_token(
		...
		contract_call!(self, child_contract_adress, ChildProxy)
			...
			.issueWrappedMoax(token_display_name, token_ticker, initial_supply)
